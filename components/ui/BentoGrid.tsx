...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText('antwibrian82@gmail.com');
    setCopied(true)
  }
    ...
    >
      <div className={`${id === 6 && "flex justify-center"} h-full`}>
        ...
        <div
          className={cn(
            titleClassName,
            "group-hover/bento:translate-x-2 transition duration-200 relative flex flex-col px-5 p-5 lg:p-10 md:h-full min-h-40"
          )}
        >
          <div className="font-sans font-extralight text-[#c1c2d3] md:text-xs z-10 lg:text-base text-sm">
            {description}
          </div>
            ...
            <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
              <div className="flex flex-col gap-3 lg:gap-8">
                {["React.js", "Next.js", "Typescript"].map((item) => (
                  <span
                    key={item}
                    className="py-2 lg:py-4 lg:px-3 px-3 text-xs opacity-50 lg:opacity-100 text-center rounded-lg bg-[#10132E]"
                  >
                    {item}
                  </span>
                ))}
